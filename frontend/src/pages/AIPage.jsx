...
import { aiApi, taskApi, channelApi, parentApi, financeApi, orgApi } from '@/lib/api';
...
export default function AIPage() {
  ...
  const [publishedAnnouncementKeys, setPublishedAnnouncementKeys] = useState(new Set());

  const handleExecuteBroadcastAnnouncement = async (proposal, proposalKey) => {
    try {
      toast.loading('Publishing announcement to campus...');
      const channels = await channelApi.list(currentOrg.id).catch(() => []);
      let targetChannel = (channels || []).find((c) => c.type === 'ANNOUNCEMENT' || c.name?.toLowerCase().includes('announcement'));
      if (!targetChannel) {
        targetChannel = (channels || []).find((c) => c.name?.toLowerCase() === 'general' || c.type === 'PUBLIC') || channels[0];
      }

      if (!targetChannel) {
        toast.dismiss();
        toast.error('No announcement or general channel found.');
        return;
      }

      const announcementText = `📢 **${proposal.title || 'Official Campus Circular'}**\n\n${proposal.content || ''}`;
      await channelApi.sendMessage(targetChannel.id, { content: announcementText });
      setPublishedAnnouncementKeys((prev) => new Set(prev).add(proposalKey));
      toast.dismiss();
      toast.success(`Announcement posted to #${targetChannel.name}! 📢`);
    } catch (e) {
      toast.dismiss();
      toast.error(e?.response?.data?.error || e?.message || 'Failed to post announcement');
    }
  };
  ...
  const renderAIMessageContent = (msgObj) => {
    ...
    const broadcastMatch = content.match(/```json\s*(\{[\s\S]*?"action"\s*:\s*"broadcast_announcement"[\s\S]*?\})\s*```/);
    
    let proposal = null;
    let contactProposal = null;
    let tallyProposal = null;
    let broadcastProposal = null;
    ...
    if (jsonMatch) {
      ...
      try {
        broadcastProposal = JSON.parse(broadcastMatch[1]);
        textOnly = content.replace(broadcastMatch[0], '').trim();
      } catch (e) {}
    }

    const proposalKey = msgObj.id || proposal?.title || broadcastProposal?.title || 'prop-key';
    const isAlreadyAssigned = assignedProposalKeys.has(proposalKey);
    const isAlreadyBroadcast = publishedAnnouncementKeys.has(proposalKey);
        ...
        {broadcastProposal && (
          <Card className="border border-amber-500/30 bg-amber-500/10 p-3.5 rounded-xl space-y-2.5 text-xs">
            ...
            {broadcastProposal.content && (
              <div className="p-2.5 rounded-lg bg-background/80 border border-border text-foreground text-xs whitespace-pre-wrap max-h-40 overflow-y-auto">
                {broadcastProposal.content}
              </div>
            )}
            <Button
              size="sm"
              disabled={isAlreadyBroadcast}
              onClick={() => handleExecuteBroadcastAnnouncement(broadcastProposal, proposalKey)}
              className={`w-full h-8 font-bold text-xs shadow-md mt-1 transition-all ${
                isAlreadyBroadcast
                  ? 'bg-muted text-muted-foreground cursor-not-allowed border border-border opacity-70'
