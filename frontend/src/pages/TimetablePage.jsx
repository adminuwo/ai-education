...
export default function TimetablePage() {
  ...
  return (
    <div className="space-y-6 text-foreground">
        ...
        {isStudent ? (
          <>
            <div className="bg-card border border-border p-5 rounded-2xl shadow-sm text-card-foreground">
              ...
              <div className="text-xs text-muted-foreground mt-1">High School Academic Wing</div>
            </div>
            ...
            <div className="bg-card border border-border p-5 rounded-2xl shadow-sm text-card-foreground">
              ...
              <div className="text-xs text-muted-foreground mt-1">Scheduled Subject Classes</div>
            </div>
            ...
            <div className="bg-card border border-border p-5 rounded-2xl shadow-sm text-card-foreground">
              ...
              <div className="text-xs text-muted-foreground mt-1">Proxy Substitutes Assigned</div>
            </div>
          </>
        ) : (
          <>
            ...
            <div className="bg-card border border-border p-5 rounded-2xl shadow-sm text-card-foreground">
              ...
              <div className="text-xs text-muted-foreground mt-1">Requiring HOD/Principal substitute</div>
            </div>
            ...
            <div className="bg-card border border-border p-5 rounded-2xl shadow-sm text-card-foreground">
              ...
              <div className="text-xs text-muted-foreground mt-1">Available for proxy assignment</div>
            </div>
          </>
        )}
      </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            {['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'].map((d) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all flex items-center gap-1 ${
                  selectedDay === d ? 'bg-primary text-primary-foreground shadow-sm font-semibold' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                }`}
              >
                <span>{d.slice(0, 3)}</span>
                {d === todayInfo.currentDayName && (
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block animate-pulse" title="Today" />
                )}
              </button>
            ))}
          </div>
          ...
          {!isStudent && (
            <select
              ...
              className="bg-background border border-input rounded-xl px-3 py-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-75"
            >
      ...
      {activeTab === 'grid' && (
        <div className="bg-card border border-border rounded-2xl p-6 space-y-4 shadow-sm text-card-foreground">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-primary" /> {selectedDay} Schedule Grid
            </h3>
            <span className="text-xs text-muted-foreground">Showing {displayedSlots.length} period slots</span>
          </div>

          <div className="overflow-x-auto rounded-xl border border-border">
            <table className="w-full text-left text-sm text-foreground">
              <thead className="bg-muted/60 text-muted-foreground uppercase text-xs font-semibold">
                <tr>
              ...
              <tbody className="divide-y divide-border">
                {displayedSlots.length === 0 ? (
                  <tr>
                    <td colSpan={isManagement ? 7 : 6} className="p-6 text-center text-muted-foreground">
                      {isTeacher
                  ...
                  displayedSlots.map((s) => (
                    <tr key={s.id} className="hover:bg-muted/40 transition-colors">
                      <td className="p-3.5 font-medium text-foreground">
                        <div className="flex items-center gap-2">
                          <span className="px-2 py-0.5 text-xs font-bold bg-primary/10 text-primary border border-primary/20 rounded-md">
                            P{s.periodNumber}
                          </span>
                          <span className="text-xs text-muted-foreground">{s.startTime} - {s.endTime}</span>
                        </div>
                      </td>
                      <td className="p-3.5 font-semibold text-foreground">{s.className}</td>
                      <td className="p-3.5 text-primary font-medium">{s.subjectName}</td>
                      <td className="p-3.5 text-muted-foreground font-mono text-xs">{s.roomNumber || 'Room 101'}</td>
                      <td className="p-3.5 text-foreground font-medium">{s.primaryTeacherName}</td>
                      <td className="p-3.5">
                        {s.isProxyAssigned ? (
                          <div className="flex items-center gap-2">
                            <span className="px-2.5 py-1 text-xs font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 rounded-full flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3" /> Proxy: {s.proxyInfo?.substituteTeacherName}
                            </span>
                            <span className="text-[10px] text-muted-foreground font-mono">({s.proxyInfo?.assignedByRole})</span>
                          </div>
                        ) : absences.some((a) => a.teacherName === s.primaryTeacherName) ? (
                          <div className="flex items-center gap-2">
                            {selectedDayInfo.isPast ? (
                              <span className="px-2.5 py-1 text-xs font-semibold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30 rounded-full flex items-center gap-1">
                                <AlertTriangle className="w-3 h-3" /> Teacher Was Absent
                              </span>
                            ) : (
                              <>
                                <span className="px-2.5 py-1 text-xs font-semibold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/30 rounded-full flex items-center gap-1">
                                  <AlertTriangle className="w-3 h-3" /> Teacher Absent
                                </span>
                                {isManagement && (
                                  <button
                                    onClick={() => handleOpenAssignProxy(s)}
                                    className="px-2 py-0.5 text-[11px] bg-primary hover:bg-primary/90 text-primary-foreground rounded-md font-medium shadow-xs"
                                  >
                                    Assign Proxy
                                  </button>
                                )}
                          ...
                          <span className="text-xs text-muted-foreground">Regular Class</span>
                        )}
                      </td>
                      {isManagement && (
                        <td className="p-3.5 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              ...
                              className="p-1.5 bg-muted hover:bg-muted/80 text-primary rounded-lg transition-all"
                            ...
                            <button
                              ...
                              className="p-1.5 bg-muted hover:bg-muted/80 text-rose-500 rounded-lg transition-all"
      ...
      {activeTab === 'free' && isManagement && (
        <div className="bg-card border border-border rounded-2xl p-6 space-y-4 shadow-sm text-card-foreground">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
                <Users className="w-5 h-5 text-emerald-500" /> Available Free Teachers ({selectedDay})
              </h3>
                ...
                {periods.map((p) => (
                  <button
                    ...
                    className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                      selectedPeriod === p ? 'bg-emerald-600 text-white shadow-sm' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                    }`}
                  >
                    P{p}
                  </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-2">
            {freeTeachers.length === 0 ? (
              ...
              freeTeachers.map((teacher) => (
                <div
                  key={teacher.id}
                  className="bg-background border border-border hover:border-emerald-500/50 p-4 rounded-xl space-y-3 transition-all shadow-xs"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="text-sm font-bold text-foreground">{teacher.name}</div>
                      <div className="text-xs text-muted-foreground">{teacher.departmentName}</div>
                    </div>
                    <span className="px-2 py-0.5 text-[10px] font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 rounded-full">
                      {teacher.role}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs pt-2 border-t border-border">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg text-xs font-medium flex items-center gap-1 shadow-xs"
                    >
      ...
      {activeTab === 'absences' && isManagement && (
        <div className="bg-card border border-border rounded-2xl p-6 space-y-6 shadow-sm text-card-foreground">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <div>
              <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
                <AlertTriangle className="w-5 h-5 text-rose-500" /> Substitute Oversight & Unassigned Periods
              </h3>
              <p className="text-xs text-muted-foreground mt-1">
            ...
            <button
              ...
              className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-medium rounded-xl flex items-center gap-1.5 shadow-sm"
            >
          ...
          <div className="space-y-4">
            <h4 className="text-sm font-semibold text-foreground">Unassigned Period Slots Requiring Substitute ({unassignedSlots.filter((s) => !s.isAssigned).length})</h4>
                ...
                <div
                  ...
                  className={`p-4 rounded-xl border space-y-3 transition-all ${
                    slot.isAssigned
                      ...
                      : 'bg-background border-rose-500/40 shadow-sm'
                  }`}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-foreground">{slot.className}</span>
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                        Subject: <span className="text-primary font-medium">{slot.subjectName}</span> • {slot.roomNumber}
                      </div>
                    </div>
                    {slot.isAssigned ? (
                      ...
                      <span className="px-2.5 py-1 text-xs font-semibold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/30 rounded-full">
                        Action Required
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs pt-2 border-t border-border">
                    <span className="text-muted-foreground">
                      Absent Teacher: <strong className="text-rose-600 dark:text-rose-400">{slot.primaryTeacherName}</strong>
                    </span>
                    {!slot.isAssigned ? (
                      <button
                        onClick={() => handleOpenAssignProxy(slot)}
                        className="px-3 py-1.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-medium text-xs flex items-center gap-1 shadow-xs"
                      >
        ...
        <DialogContent className="sm:max-w-md bg-card border-border text-card-foreground">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg font-semibold text-foreground">
              <AlertTriangle className="w-5 h-5 text-rose-500" /> Report Teacher Leave
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleReportAbsence} className="space-y-3">
            <div>
              <div className="flex items-center justify-between pb-1">
                <label className="text-xs font-medium text-foreground">Select Faculty / Teacher</label>
              ...
              <select
                ...
                className="w-full bg-background border border-input rounded-xl p-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-rose-500 cursor-pointer"
              >
            ...
            <div>
              <div className="flex items-center justify-between pb-1">
                <label className="text-xs font-medium text-foreground">Date of Absence</label>
                {newAbsence.date && (() => {
              ...
              <input
                ...
                className="w-full bg-background border border-input rounded-xl p-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-rose-500 cursor-pointer"
              />
              <p className="text-[11px] text-muted-foreground mt-1.5">Absence will be marked on the timetable for this date only.</p>
            </div>
      ...
      <Dialog open={showAssignProxyModal} onOpenChange={setShowAssignProxyModal}>
        <DialogContent className="sm:max-w-md bg-card border-border text-card-foreground">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg font-semibold text-foreground">
              <UserPlus className="w-5 h-5 text-primary" /> Assign Substitute Teacher
            </DialogTitle>
          </DialogHeader>
          {selectedSlotForProxy && (
            <div className="bg-muted/30 p-3 rounded-xl border border-border text-xs text-foreground space-y-1">
              <div>Class: <strong className="text-foreground">{selectedSlotForProxy.className}</strong></div>
              <div>Subject: <strong className="text-primary">{selectedSlotForProxy.subjectName}</strong> (Period {selectedSlotForProxy.periodNumber})</div>
              <div>Absent Teacher: <strong className="text-rose-600 dark:text-rose-400">{selectedSlotForProxy.primaryTeacherName}</strong></div>
            </div>
          )}
          <form onSubmit={handleAssignProxy} className="space-y-3">
            <div>
              <div className="flex items-center justify-between pb-1">
                <label className="text-xs font-medium text-foreground">Select Substitute Teacher</label>
                <span className="text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full border bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30">
                  Free for Period {selectedSlotForProxy?.periodNumber}
                </span>
              </div>
              {loadingProxyTeachers ? (
                <div className="w-full bg-background border border-input rounded-xl p-3 text-sm text-muted-foreground flex items-center gap-2">
                  <RefreshCw className="w-3.5 h-3.5 animate-spin text-primary" /> Loading available teachers...
                </div>
              ) : slotFreeTeachers.length === 0 ? (
                <div className="w-full bg-background border border-amber-500/30 rounded-xl p-3 text-xs text-amber-600 dark:text-amber-400 flex items-center gap-2">
                ...
                <select
                  ...
                  className="w-full bg-background border border-input rounded-xl p-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary cursor-pointer"
                >
            ...
            <div>
              <label className="text-xs font-medium text-foreground">Notes for Proxy Teacher</label>
              <input
                type="text"
                placeholder="e.g. Conduct Chapter 4 revision"
                value={proxyForm.notes}
                onChange={(e) => setProxyForm({ ...proxyForm, notes: e.target.value })}
                className="w-full bg-background border border-input rounded-xl p-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
            <DialogFooter className="flex items-center justify-end gap-3 pt-3">
              <button
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium rounded-xl"
              >
                Assign Substitute
              </button>
            </DialogFooter>
          ...
          <form onSubmit={handleSaveSlot} className="space-y-4 py-2">
            ...
            {slotConflictError && (
              <div className="bg-rose-500/10 border border-rose-500/40 rounded-xl p-3 space-y-1">
                ...
                <p className="text-xs text-rose-500/90 dark:text-rose-300 leading-relaxed">{slotConflictError.message}</p>
                {slotConflictError.existing && (
                  <div className="bg-background border border-border rounded-lg p-2 mt-1 text-[11px] text-muted-foreground space-y-0.5">
                    <div>📚 <span className="text-foreground">{slotConflictError.existing.subjectName}</span> — {slotConflictError.existing.className}</div>
                    <div>👤 <span className="text-foreground">{slotConflictError.existing.primaryTeacherName}</span></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-foreground">Class / Section</label>
                <select
                  required
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="text-xs font-medium text-foreground">Period No.</label>
                <select
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs font-medium text-foreground">Classroom / Lab</label>
                <input
                  type="text"
            ...
            <div>
              <label className="text-xs font-medium text-foreground">Primary Assigned Teacher</label>
              <select
                required
